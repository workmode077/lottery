...
use Modules\Admin\Http\Requests\AgentPostRequest;
...
class AgentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            )->where('user_type', 'agent')->with('superAgent'); 
    ...
    public function create()
    {
        try {
            $baseRouteName  = $this->getBaseRouteName();
            $parentAgent = User::where('user_type', 'super_agent')->active()->get();
            if (!$baseRouteName) {
                return abort(404);
            }
    ...
    public function store(AgentPostRequest $request)
    {
        ...
        try {
            ...
            // Ensure prefix
            if (!str_starts_with($fillableData['username'], 'AG-')) {
                $fillableData['username'] = 'AG-' . $fillableData['username'];
            }

            $fillableData['user_type'] = 'agent';
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(AgentPostRequest $request, $id)
    {
        DB::beginTransaction();
        try {
