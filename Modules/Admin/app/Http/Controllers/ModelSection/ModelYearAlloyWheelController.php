...
use Modules\Admin\Http\Requests\ModelYearAlloyWheelPostRequest;
use Yajra\DataTables\Facades\DataTables;

class ModelYearAlloyWheelController extends Controller
{
    protected $modelClass = 'App\Models\VehicleAlloyColor';
    protected $viewPath = 'alloy-wheel';
    /* ============ DATATABEL ============= */
        ...
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($data)
                ...
                ->addColumn('image', function ($row) {
                    return '<img src="' . $row->thumbnail_url . '" alt="' . $row->name . '" class="table-thumbnail">';
                })
                ->addColumn('action', function ($row) use ($baseRouteName, $modelyearid) {
                    $editUrl = route($baseRouteName . '.edit', [
                        ...
                        'alloywheel' => base64_encode($row->id)
                    ]);
    ...
    /* ============ STORE FUNCTION  ============= */
    public function store(ModelYearAlloyWheelPostRequest $request, $model_year)
    {
        DB::beginTransaction();
        try {
            $input = $request->all();
            ...
            $data->uploadMedia($request, 'thumbnail');
    ...
    /* ============ EDIT PAGE  ============= */
    public function edit($model_year, $alloywheel)
    {
        try {
            $modelyearid = base64_decode($model_year);
            $highlightId = base64_decode($alloywheel);
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(ModelYearAlloyWheelPostRequest $request, $model_year, $id)
    {
        DB::beginTransaction();
        try {
