...
use Modules\Admin\Http\Requests\ModelYearPerformancePostRequest;
use Yajra\DataTables\Facades\DataTables;

class ModelYearPerformanceController extends Controller
{
    protected $modelClass = 'App\Models\VehiclePerformance';
    protected $viewPath = 'performance';
    /* ============ DATATABEL ============= */
            ...
            return DataTables::eloquent($data)
                ...
                ->addColumn('action', function ($row) use ($baseRouteName, $modelyearid) {
                    $editUrl = route($baseRouteName . '.edit', [
                        ...
                        'performance' => base64_encode($row->id)
                    ]);
    ...
    /* ============ STORE FUNCTION  ============= */
    public function store(ModelYearPerformancePostRequest $request, $model_year)
    {
        DB::beginTransaction();
        try {
            $input = $request->all();
    ...
    /* ============ EDIT PAGE  ============= */
    public function edit($model_year, $performance)
    {
        try {
            $modelyearid = base64_decode($model_year);
            $highlightId = base64_decode($performance);
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(ModelYearPerformancePostRequest $request, $model_year, $id)
    {
        DB::beginTransaction();
        try {
