...
use Modules\Admin\Http\Requests\ModelYearBodyFeaturePostRequest;
use Yajra\DataTables\Facades\DataTables;

class ModelYearBodyFeaturesController extends Controller
{
   protected $modelClass = 'App\Models\VehicleBodyFeature';
    protected $viewPath = 'bodyfeature';
    /* ============ DATATABEL ============= */
            ...
            return DataTables::eloquent($data)
                ...
                ->addColumn('action', function ($row) use ($baseRouteName, $modelyearid) {
                    $editUrl = route($baseRouteName . '.edit', [
                        ...
                        'bodyfeature' => base64_encode($row->id)
                    ]);
    ...
    /* ============ STORE FUNCTION  ============= */
    public function store(ModelYearBodyFeaturePostRequest $request, $model_year)
    {
        DB::beginTransaction();
        try {
            $input = $request->all();
    ...
    /* ============ EDIT PAGE  ============= */
    public function edit($model_year, $bodyfeature)
    {
        try {
            $modelyearid = base64_decode($model_year);
            $highlightId = base64_decode($bodyfeature);
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(ModelYearBodyFeaturePostRequest $request, $model_year, $id)
    {
        DB::beginTransaction();
        try {
