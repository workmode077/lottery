...
use Modules\Admin\Http\Requests\ModelYearExteriorHighlightsPostRequest;
use Yajra\DataTables\Facades\DataTables;

class ModelYearExteriorHighlightsController extends Controller
{
    protected $modelClass = 'App\Models\InteriorExteriorHighlight';
    protected $viewPath = 'exterior-highlights';
    /* ============ DATATABEL ============= */
    public function index(Request $request, $model_year)
    {
        ...
        if ($request->ajax()) {
            $data = $this->modelClass::where('model_year_id', $modelyearid)->where('type', 'exterior-highlight')->when(
            ...
            return DataTables::eloquent($data)
                ...
                ->addColumn('action', function ($row) use ($baseRouteName, $modelyearid) {
                    $editUrl = route($baseRouteName . '.edit', [
                        ...
                        'exterior_highlight' => base64_encode($row->id)
                    ]);
    ...
    /* ============ STORE FUNCTION  ============= */
    public function store(ModelYearExteriorHighlightsPostRequest $request, $model_year)
    {
        DB::beginTransaction();
        try {
            $input = $request->all();
    ...
    /* ============ EDIT PAGE  ============= */
    public function edit($model_year, $exterior_highlight)
    {
        try {
            $modelyearid = base64_decode($model_year);
            $highlightId = base64_decode($exterior_highlight);
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(ModelYearExteriorHighlightsPostRequest $request, $model_year, $id)
    {
        DB::beginTransaction();
        try {
