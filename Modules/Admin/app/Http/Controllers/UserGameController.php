...
use Modules\Admin\Http\Requests\UserGameRequest;
use Yajra\DataTables\Facades\DataTables;

class ModelYearInteriorController extends Controller
{
    protected $modelClass = 'App\Models\UserGame';
    protected $viewPath = 'interior';
    /* ============ DATATABEL ============= */
    public function index(Request $request, $model_year)
    {
        ...
        if ($request->ajax()) {
            $data = $this->modelClass::where('model_year_id', $modelyearid)->where('type', 'interior')->when(
        ...
        return view('admin::sub-agent.user-games.' . $this->viewPath . '.index', compact('baseRouteName', 'modelyearid','modelyear'));
    }
    ...
    public function create($model_year)
    {
        try {
            $modelyearid = base64_decode($model_year);
            $baseRouteName = $this->getBaseRouteNameSub();
            $sort_order =  $this->modelClass::where('type','interior')->max('sort_order') + 1;

            if (!$baseRouteName) {
                return abort(404);
            }

            return view('admin::sub-agent.user-games.' . $this->viewPath . '.create', compact('baseRouteName', 'modelyearid', 'sort_order'));
        } catch (\Exception $e) {
    ...
    /* ============ STORE FUNCTION  ============= */
    public function store(UserGameRequest $request, $model_year)
    {
        DB::beginTransaction();
        try {
            $input = $request->all();
    ...
    /* ============ EDIT PAGE  ============= */
    public function edit($model_year, $interior)
    {
        try {
            $modelyearid = base64_decode($model_year);
            $highlightId = base64_decode($interior);
            ...
            return view('admin::sub-agent.user-games.' . $this->viewPath . '.create', compact('data', 'baseRouteName', 'modelyearid'));
        } catch (\Exception $e) {
            Log::error($e);
            return response()->view('admin::errors.500', [], 500);
        }
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(UserGameRequest $request, $model_year, $id)
    {
        DB::beginTransaction();
        try {
