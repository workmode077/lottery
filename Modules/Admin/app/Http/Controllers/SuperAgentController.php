...
class SuperAgentController extends Controller
{
    ...
    /* ============ STORE FUNCTION  ============= */
    public function store(SuperAgentPostRequest $request)
    {
        DB::beginTransaction();
        try {
            ...
            // Ensure prefix
            if (!str_starts_with($fillableData['username'], 'SA-')) {
                $fillableData['username'] = 'SA-' . $fillableData['username'];
            }

            $fillableData['user_type'] = 'super_agent';
            $fillableData['password'] = bcrypt($fillableData['plain_password']);
    ...
    /* ============ UPDATE FUNCTION  ============= */
    public function update(SuperAgentPostRequest $request, $id)
    {
        DB::beginTransaction();
        try {
            ...
            if (!empty($fillableData['plain_password'])) {
                $fillableData['password'] = bcrypt($fillableData['plain_password']);
            }
